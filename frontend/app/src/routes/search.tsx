        ...
        <div style={{ padding: 14 }}>
            ...
            <div className="panel" style={{ marginBottom: 14 }}>
              ...
              <div style={{ padding: 14 }}>
                ...
                >
                  ...
                  <b style={{ color: "var(--color-fg-0)" }}>any</b> file generated by
                  analysis, including dropped/extracted files).
                </p>
                <p
                  className="dim"
                  style={{ fontSize: 12, lineHeight: 1.6, marginTop: 0, marginBottom: 6 }}
                >
                  To search for the <b style={{ color: "var(--color-fg-0)" }}>initial submitted file</b>{" "}
                  specifically, use{" "}
                  <code className="mono" style={{ color: "var(--color-accent)" }}>
                    target_sha256:
                  </code>{" "}
          ...
          {submittedTerm && (
            ...
            >
              Results for term:{" "}
              <span
                style={{ color: "var(--color-sev-crit)", fontWeight: 600 }}
                className="mono"
              >
                {submittedTerm}
              </span>
          ...
          {search.data?.ok && search.data.items.length > 0 && (
            <div className="panel">
              <div className="panel-h">
                ...
                Search Results
                <span className="count">· {search.data.items.length} item{search.data.items.length === 1 ? "" : "s"}</span>
              </div>
          ...
          {search.data?.ok &&
            ...
            !search.data.error && (
              ...
              >
                <SearchIcon
                  size={36}
                  style={{ color: "var(--color-fg-3)", marginBottom: 10 }}
                />
      ...
      {items.map((p) => (
        <tr key={p.prefix}>
          <td style={{ textAlign: "center" }}>
            <code
              className="mono"
              style={{ color: "var(--color-accent)", fontSize: 11.5 }}
            >
              {p.prefix}:
            </code>
          </td>
...
function highlightInline(s: string): string {
  ...
  return s.replace(
    /<code>([^<]+)<\/code>|`([^`]+)`/g,
    (_m, a, b) =>
      `<code class="mono" style="color: var(--color-accent)">${a ?? b}</code>`,
  );
}
